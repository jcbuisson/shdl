...
export function useBusinessObservables(app) {
   ...
   const { getObservable: groupSlotSHDLTestRelation$ } = useGroupSlotSHDLTestRelation(app)
   ...
   function isTeacher$(user_uid: string) {
   ...
   function userSHDLTests$(user_uid: string) {
   ...
   function userGroupSlotSHDLTestRelation$(user_uid: string) {
      return userSlots$(user_uid).pipe(
         switchMap(slotList =>
            guardCombineLatest(
               slotList.map(slot => groupSlotSHDLTestRelation$({ group_slot_uid: slot.uid }))
   ...
   function userSHDLTestsRelations$(user_uid: string) {
   ...
   // emit value in 0..100
   function userTestGrade$(user_uid: string) {
      return guardCombineLatest([
   ...
   // emit value in 0..100
   function userAttendanceGrade$(user_uid: string, now) {
      return guardCombineLatest([
         userSlots$(user_uid),
         userEvents$(user_uid),
   ...
   function shdlDocumentParsing$(user_uid, name, checked=[]) {
      return userDocument$({ user_uid, name }).pipe(
         ...
         switchMap(({ document, structure, submoduleNames }) => {
            ...
            structure.document_name = document.name
