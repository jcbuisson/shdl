...
export function useModel(app) {
   ...
   function createModel(dbName: string, modelName: string, fields) {
      ...
      function getObservable(where = {}) {
         ...
         //       // L'EXÉCUTION DANS DES WORKERS COMPLIQUE ET NE SEMBLE PAS APPORTER DE GAIN EN VITESSE
         //       if (workerApi) {
         ...
         //    }
         // })
         // read database and update indexedDB asynchronously
         app.service(modelName).findMany({ where }).then(async (values) => {
            for (const value of values) {
               // const x = await db.values.update(value.uid, value)
               try {
                  await db.values.add(value)
               } catch(err) {
                  await await db.values.update(value.uid, value)
               }
            }
         })
         const predicate = wherePredicate(where)
         return from(liveQuery(async () => {
            const values = await db.values.filter(value => !value.__deleted__ && predicate(value)).toArray();
            return values;
         }))
      }
      ...
      //    promise.then(async isNew => {
      //       if (isNew) {
      //          const list = await db.whereList.toArray();
      //          console.log(`addSynchroWhere (${isNew})`, dbName, modelName, where, list.length)
      //       }
      //    })
      //    return promise
      // }
