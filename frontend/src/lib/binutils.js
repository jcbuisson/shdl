...
function padStart(string, length, chars) {
   string = String(string); // Ensure the input is treated as a string
   length = Math.floor(length) || 0; // Ensure length is a non-negative integer
   chars = String(chars === undefined ? ' ' : chars); // Default pad character is a space
   ...
   // If the pad is longer than needed, truncate it
   pad = pad.slice(0, padLength);
