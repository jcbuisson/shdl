...
export function checkModule(moduleName, name2module) {

   // peg parse of `module` and all its submodules
   let { moduleList, err: pegError } = pegParseDeep(moduleName, name2module)
   if (pegError) {
      return {
         err: {
            module: moduleName,
   ...
   let syntaxError = checkStructure(moduleName, name2module)
   if (syntaxError) {
      return {
         err: {
            module: moduleName,
   ...
   for (let i = 0; i < moduleList.length; i++) {
      ...
      let instanceError = collectEquipotentials(submodule.name, name2module)
      if (instanceError) {
