...
const utilities = require('./linked-utilities/shdlUtilities.js')
const binutils = require('./linked-utilities/binutils.js')
...
module.exports = (function() {
   ...
   function boardSignals(board, equipotentials, equipotentialIndex) {
      let names = utilities.getEquipotentialNames(equipotentials, equipotentialIndex).map(name => name.substring(2))
   ...
   async function synthesize(moduleName, userId, teamId, board, server, vivadoPath, jwt, options) {
      console.log(`Synthesis of module '${moduleName}' for board '${board.name}'`)
      let moduleFile = `${moduleName}-${uuidv1()}`

      // fetch root module tree from server
      let name2module = await fetcher.fetchModuleTreeFromServer(moduleName, userId, teamId, server, jwt, options)
      ...
      let parameterCount = utilities.parametersArity(parameterList)

      // verify that all input/output are board signals and build .xdc file content
      let xdc = ''
      for (let i = 0; i < parameterCount; i++) {
         let parameterName = utilities.parametersNameAtIndex(parameterList, i)
...
`
      ...
      fsp.writeFile(`/tmp/${moduleFile}.xdc`, xdc)
      ...
      if (options.memfile) {
         let memoryFileContents = await fsp.readFile(options.memfile)
         ...
         if (contentsDict !== null) {
            ...
            for (let addr = Math.pow(2, ramblock.addrs.length) - 1; addr >= 0; addr--) {
               ...
               let svalue = (value === undefined)
                  ...
                  : binutils.strToBin(value, ramblock.dout.length)
               values.push(svalue)
            }
...
`
      ...
      fsp.writeFile(`/tmp/${moduleFile}.vhd`, vhdl)
...
`
      fsp.writeFile(`/tmp/${moduleFile}.tcl`, tclScript)
