...
//          // throw exception (useful when used as database service hook)
...
// // If authenticated and not expired: extend expiration date of `duration`
// // If not, send 'not-authenticated' event to client and throws an error (blocks calls when used as 'before' database service hook)
// export const checkAuthenticationAndExtendExpiration = (duration) => async (context) => {
//    // do nothing if it's not a client call from a ws connexion
//    if (!context.socket) return
//    const expiresAt = context.socket?.data?.expiresAt
//    if (expiresAt) {
...
//          context.socket.emit('not-authenticated')
//          // throw exception (blocks calls when used as 'before' database service hook)
//          throw new EXError('not-authenticated', "Session expired")
//       } else {
//          // extend expiration
//          context.socket.data.expiresAt = new Date(now.getTime() + duration)
